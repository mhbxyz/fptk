...
def test_writer_map_bind_run() -> None:
    ...
    w2 = Writer.unit(3, monoid_list).bind(
        lambda x: tell([f"processed {x}"], monoid_list).map(lambda _: x * 2)  # pyright: ignore[reportArgumentType]
    )
    ...
    w3 = Writer.unit(2, monoid_list).bind(
        lambda x: tell([f"step1: {x}"], monoid_list).bind(  # pyright: ignore[reportArgumentType]
            lambda _: tell([f"step2: {x}"], monoid_list).map(lambda _: x + 10)  # pyright: ignore[reportArgumentType]
        )
    )
...
def test_tell_listen_censor() -> None:
    ...
    # censor modifies log
    w3 = censor(lambda logs: [log.upper() for log in logs], tell(["hello"], monoid_list))  # pyright: ignore[reportArgumentType]
